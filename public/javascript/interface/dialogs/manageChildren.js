...
((window, document) => {
  ...
  //  ADDING A CHILD
  $("#addChild").click(e => {
    //  when #addChild is clicked
    showModal("addChild.html#form", "30%", async () => {
      //  change the size of the modal accordingly
      ...
      availableWatches.rows.forEach(item => {
        //  fill the watches in a select panel
      ...
      $("#leftButton").click(async e => {
        ...
        try {
          const child = await createChild({
            //  try to create child //  from db/actions/child.js
            Parent: parent,
          ...
          //  on fail
          newToast(
            `failed to add "${$("#childName").val()}".\n ${err.message}`
          ); //  show failure in toast. //  from interface/toast.js
        } finally {
          hideLoading(false); //  hide loading screen but keep the container. //  from interface/modal.js
  ...
  //  REMOVING A CHILD
  $("#removeChild").click(e => {
    //  when #removeChild is clicked
    showModal(
      "removeChild.html#form",
      "25%",
      async () => {
        //  change the size of the modal accordingly
        $("#leftButton").click(async e => {
          ...
          try {
            const childToRemove = await destroyChild({
              //  try to delete child //  from db/actions/child.js
              name: child
            });
            //  on success
            ...
            $("#currentChild")
              .empty()
              .trigger(events.childListUpdated); //  fire event to update droplist
      ...
      () => {
        $("#childDeletion").html(
          //  to alter the message in html/components/modals/removeChild.html
  ...
  $("#currentChild").change(e => {
    //  when the user chooses an item from the droplist
    ...
    localStorage.setItem(
      "currentSmartwatch",
      dataStorage.childList[childIdx].Smartwatch
    ); //  saving the smartwatch
